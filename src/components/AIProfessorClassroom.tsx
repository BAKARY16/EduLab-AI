...
import type { CatalogLesson } from "@/server/courseCatalog";
...
type TutorResult = { answer?: string; detail?: string; checkpoint?: Checkpoint; web_research_used?: boolean; sources?: { title?: string; source_id?: string; url?: string }[] };
type Mode = "lesson" | "chapter";
type VoiceIntent = "question" | "answer";

export function AIProfessorClassroom({ lesson, chapterLessons }: { lesson: CatalogLesson; chapterLessons: string[] }) {
  ...
  const elapsedSec = () => Math.max(1, Math.round((Date.now() - promptShownAtRef.current) / 1000));
  ...
  async function ask(instruction: string, nextState: RobotState = "parole", onSpeechFinished?: () => void) {
    stopVoice(); setLoading(true); setState("reflexion");
    try {
      const response = await fetch("/api/teacher-model", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ instruction, context: board.slice(0, 3000), class_name: lesson.class_name, subject: lesson.subject, max_new_tokens: 80 }) });
      ...
      setBoard(answer); setSources(result.sources || []); setState(nextState);
      setCheckpoint(result.checkpoint || null); setSelectedChoice(null);
      speechFinishedRef.current = onSpeechFinished || null;
      if (result.answer) {
        if (voiceOn) void speak(result.answer);
        else if (onSpeechFinished) window.setTimeout(onSpeechFinished, 1200);
      }
  ...
  function presentChapterLesson(index: number) { const title=chapterLessons[index]; if(!title) return; setChapterStep(index); setPendingChapterStep(index + 1 < chapterLessons.length ? index + 1 : null); promptShownAtRef.current=Date.now(); void ask(`Fais un exposé très résumé de le cours « ${title} » pour ${lesson.class_name}. Donne l’objectif, 3 à 5 idées essentielles, les formules indispensables, un exemple calculé et une question de vérification. Appuie-toi d'abord sur le RAG; s'il est insuffisant, utilise la recherche autorisée et distingue clairement ces sources.`); }
  function startLesson() {
    ...
    setStarted(true); promptShownAtRef.current=Date.now();
    if(mode === "chapter") { presentChapterLesson(0); return; }
    void ask(`Fais un exposé très résumé de ${scope} pour ${lesson.class_name}. Donne l’objectif, 3 à 5 idées essentielles, les formules indispensables, un exemple calculé et une question de vérification. Appuie-toi d'abord sur le RAG; s'il est insuffisant, utilise la recherche autorisée et distingue clairement ces sources.`);
  }
  function demonstrate() {
    ...
    void ask(`Agis comme un professeur de mathématiques au tableau. Pour ${scope}, choisis un calcul pertinent et écris: données, domaine, propriété utilisée, transformations une par une, vérification et conclusion. Aucun saut de calcul. Si le RAG est insuffisant, complète par une recherche autorisée et cite-la.`, "ecriture");
  }
  function submitQuestion(event: FormEvent) {
    ...
    promptShownAtRef.current=Date.now();
    void ask(`Dans le cadre de ${scope}, réponds simplement à cette question : ${value}. Commence par les cours indexés; si l'information manque, cherche dans les domaines autorisés. Cite les sources et indique clairement ce qui provient du web.`);
  }
  ...
  function browserVoiceFallback() {
    ...
    recognition.onresult = event => { const text = event.results[0][0].transcript; setQuestion(text); if(voiceIntent === "answer" && checkpoint){const next=pendingChapterStep; void ask(`Réponse orale de l'apprenant à « ${checkpoint.question} »: ${text}. Évalue-la et explique la correction.`, "correction", next === null ? undefined : () => presentChapterLesson(next));} else void ask(`Question orale de l'apprenant dans le cadre de ${scope}: ${text}. Réponds naturellement et vérifie sa compréhension.`); };
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          } else await ask(`Question orale de l'apprenant dans le cadre de ${scope}: ${result.text}. Réponds naturellement, simplement et avec une vérification de compréhension.`);
  ...
  return <section className="mt-6 overflow-hidden rounded-[1.75rem] border border-[#174c3a]/15 bg-[#f5f1e7] shadow-[0_18px_50px_rgba(23,76,58,.09)]">
    ...
    <div className="p-4 sm:p-5">
      <div className="mb-4 grid gap-2 sm:grid-cols-2"><ModeButton active={mode === "lesson"} onClick={() => setMode("lesson")} icon={<BookOpen className="h-4 w-4"/>} title="Ce cours uniquement" subtitle={lesson.title}/><ModeButton active={mode === "chapter"} onClick={() => setMode("chapter")} icon={<Presentation className="h-4 w-4"/>} title="Toute la notion" subtitle={`${lesson.chapter} · ${chapterLessons.length} cours`}/></div>
